...
This replaces all previous distortion scripts.
"""
    ...
    for v in variants:
        ...
        if min_word_diff > 0:
            for existing in valid:
                ...
                # Check if below threshold (only for questions with enough words)
...
class DistortionRunner:
    ...
    def _create_preliminary_csv(self) -> pd.DataFrame:
        ...
        - subject: Subject/category of the question
        - question_id: Unique identifier for the original question
        ...
        - answer: Correct answer letter(s)
        ...
        # Load all CSV files from original_data
        ...
        print(f"   Loaded {len(combined)} questions from {len(csv_files)} files")
        ...
        # Track unique distortion_id across all questions and mius
        global_distortion_counter = 0
        ...
        print(f"   Created {len(df)} rows ({len(combined)} questions × {len(miu_values)} miu levels)")
    ...
    def _run_post_validation(self):
        ...
        1. Check for exact duplicates (same text)
        2. Check for near-duplicates (>95% word overlap between distortions)
        3. LLM Judge (Mistral) validates according to miu rules
        """
        print(f"\n{'='*60}")
        ...
        while True:
            ...
            # Step 3: Check for near-duplicates (very similar distortions)
...
ALL {N} DISTORTIONS (for this question at μ={miu}):
{distortion_block}
...
□ MEANING PRESERVED?
  - Does this distortion still ask the same thing as the original?
  - Would the correct answer to the original still be correct here?
